...
{
    gtk_init(&argc, &argv);
    GtkWidget *window, *label,*button_reserv, *button_add, *button_remove, *button_history, *button_exit, *button_week;
    window = gtk_window_new(GTK_WINDOW_TOPLEVEL);
    gtk_window_set_title(GTK_WINDOW(window), "System rezerwacji sprzetu");
    gtk_window_set_default_size(GTK_WINDOW(window), (gint)400, (gint)400);
    ...
    button_week = gtk_button_new_with_label("Plan na tydzien");
    ...
    gtk_grid_attach(GTK_GRID(grid), button_exit, 2, 11, 1, 1);
